...
# ══════════════════════════════════════════════════════
SHEET_ID       = "1ty88pWmkwi-Qs3S3MvhIR0_cmxNTI7Rlo76mpFyL2Zs"
TEMPLATE_FILE  = "dashboard.html"
OUTPUT_FILE    = "index.html"

NOME_CLIENTE   = "hLiberty"
LOGO_LETRA     = "hL"
...
# Conversão = mensagem + contato + pixel (somados, igual ao resto do dashboard)
CONV_COLS = {
    "Action Messaging Conversations Started (Onsite Conversion)": "messaging",
    "Conversion Contact Total": "contact",
    "Action FB Pixel Complete Registration (Offsite Conversion)": "pixel",
}
...
def load_meta():
    ...
    print(f"     Alcance total: {df['reach'].sum():.0f} | Engajamento total: {df['engajamento'].sum():.0f} | "
          f"Conversões total: {df['conversoes'].sum():.0f}")
...
def calc_kpis(p):
    ...
    return {
        ...
        "cpl": round(sp / conv, 2) if conv > 0 else None,
...
def build_daily(p):
    ...
    for _, r in agg.iterrows():
        sp = float(r["spend"]); imp = float(r["impressions"]); eg = float(r["engajamento"]); cv = float(r["conversoes"])
        ...
        out["conversoes"].append(int(cv))
        out["cpl"].append(round(sp / cv, 2) if cv > 0 else None)
    return out
...
def load_breakdown(url, dim_cols):
    try:
        ...
        df = pd.DataFrame(index=raw.index)
        df["date"] = pd.to_datetime(raw["Date"], errors="coerce") if "Date" in raw.columns else pd.NaT
        for label, field in dim_cols.items():
            df[field] = raw[label] if label in raw.columns else ""
        df["spend"] = _sum_dupe_cols(raw, "Spend (Cost, Amount Spent)")
        df["reach"] = _sum_dupe_cols(raw, "Reach (Estimated)")
        df["impressions"] = _sum_dupe_cols(raw, "Impressions")
        df["engagement"] = _sum_dupe_cols(raw, "Action Post Engagement")
        conv = pd.Series(0, index=raw.index)
        for label in CONV_COLS:
            conv = conv + _sum_dupe_cols(raw, label)
        df["conversoes"] = conv
        df = df.dropna(subset=["date"])
        return df
...
def build_bd_age_gender():
    ...
    for _, r in df.iterrows():
        rows.append({"d": r["date"].strftime("%d/%m/%Y"), "age": str(r.get("age", "")), "gen": str(r.get("gender", "")),
                     ...
                     "eng": int(r["engagement"]), "cv": int(r["conversoes"])})
    return rows
...
def build_raw(df, img_dir):
    ...
    for _, r in agg.iterrows():
        ...
        rows.append({
            ...
            "pv": int(r["profile_visits"]), "cv": int(r["conversoes"]),
            "cl": int(r["clicks"]),
